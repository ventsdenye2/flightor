...
const C=load('src/features/maps/TripMap.tsx',name=>{
 ...
 if(name==='./diagnostics')return {recordMapDiagnostic:()=>{}}
...
C({points,locale:'en',mapKey:'never-ready'});timers.at(-1)();assert.equal(states.at(-1),false,'missing updated is unknown, not a certified map failure')
let native
const viewport=[]
vm.runInNewContext(fs.readFileSync('src/components/place-map/index.js','utf8'),{require:()=>require('../src/components/place-map/diagnostic-detail'),setTimeout:()=>1,clearTimeout:()=>{},Component:value=>{native=value},wx:{createMapContext:()=>({includePoints:value=>viewport.push(value)})}})
const events=[],data={};const instance={data,...native.methods,setData:value=>Object.assign(data,value),triggerEvent:(...value)=>events.push(value)}
...
native.methods.selectMarker.call(instance,{detail:{markerId:2}});assert.equal(events.at(-1)[0],'selectplace')
assert.equal(events.at(-1)[1].markerId,2)
