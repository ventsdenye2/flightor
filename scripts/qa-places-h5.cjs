...
const output=path.resolve('output/playwright/'+(followup?'map-followup':'places-map'));fs.mkdirSync(output,{recursive:true})
...
;(async()=>{
 ...
 try{for(const entry of transport.entries){
  ...
  let releasePlaces,placeReadStarted=false,placeReadFinished=false
  const gate=new Promise(resolve=>{releasePlaces=resolve})
  ...
  await context.route('**/v1/**',async route=>{
   ...
   if(followup&&u.pathname.endsWith('/places')&&req.method()==='GET'){placeReadStarted=true;await gate;placeReadFinished=true}
  ...
  if(followup){assert.equal(placeReadFinished,false);await page.screenshot({path:path.join(output,`${entry.id}-text-before-places.png`)});report.textBeforePlaces=true;report.runtime=await page.evaluate(()=>globalThis.__FLIGHTOR_BUILD__);assert.equal(report.runtime.sourceFingerprint,JSON.parse(fs.readFileSync('dist-h5/build-info.json')).sourceFingerprint);releasePlaces()}
  ...
  if(!followup)await Promise.all([page.waitForResponse(r=>r.request().method()==='POST'&&r.url().endsWith('/places')),page.locator('.ux-prepare-places').click()])
  ...
  assert.equal(report.requests.filter(r=>r.entry===entry.id&&r.method==='POST').length,followup?0:1)
  report.diagnostics=await page.evaluate(()=>globalThis.__FLIGHTOR_MAP_DIAGNOSTICS__)
