...
const displayPayload = {
  ...
  formatResearchDescription: value => ({ description: typeof value === 'string' ? value : '', notice: '以下为来源资料摘要；其中价格、开放时间和交通时长的当前及出行日适用性尚未核实，请以运营方届时公告为准。' }),
  ...
  displayTravelGuidePublication: (value, expectedArtifactId, expectedTripContextVersion) => value && value.version === 1 && value.artifactId === expectedArtifactId && value.tripContextVersion === expectedTripContextVersion && value.contentContract === 'limited' && (value.evidenceCoverage === 'partial' || value.evidenceCoverage === 'unknown') && value.budgetAssessment?.status === 'undetermined' && value.budgetAssessment?.knownSubtotal === null && value.budgetAssessment?.scopeCoverage === 'incomplete' && typeof value.budgetAssessment?.notice === 'string' && typeof value.legacy === 'boolean' && typeof value.reply === 'string' ? { ...value, budgetNotice: value.budgetAssessment.notice } : undefined,
