...
The optional read-side Artifact envelope enrichment is passed through only for the matching accepted guideContentHash and stable activity ID. Original city identity clues remain `locationHint`; city center is never a venue coordinate. Stage two added the extension transport only; stage three supplies independently persisted place identities via explicit authenticated GET/POST, without mutating the protected guide body. See [ADR 0026](adr/0026-place-identity-and-maps.md) for place records, cache and version guards. Stage four adds independent Wikimedia media GET/POST, migration 014, activity photo bindings and cache: [ADR 0027](adr/0027-place-media.md). The client merges only the arriving extension fields; source text/publication and business revisions stay unchanged. Media completion is explicit, outside Planner and localization, and never needs a successful map render. WeChat basemap investigation is paused and remains unresolved; H5 media evidence does not close map or G1 acceptance.
# DSH 可选后端底座（2026-09-24）

实验分支 `codex/dsh-backend` 保留当前公开认证API和前端，抽出四方法 PlannerServicePort；legacy使用旧循环，DSH通过独立私有IPC worker执行官方AgentLoop。两个service只共享领域仓库、工具和PlannerDomainService（owner校验、版本读取与显式本地化）；DSH不得调用旧CloudPlannerService.runTurn或AgentRuntime.run。

SessionManager为应用级、有界、单API实例对象，serviceForUser仍可按请求创建轻量owner作用域服务。轮询/GET不创建Agent；同会话只允许一条活跃turn。Session JSONL与父进程scope映射必须持久备份，Memory版本变化退休旧执行历史。取消先撤销generation、中止worker并等待父领域操作实际结束，再让现有turn store确认取消。部署配置和回滚见[部署说明](deploy.md)，验证范围见[DSH记录](design/budget-travel-agent/DSH_IMPLEMENTATION_REPORT_2026-09-24.md)。G1未关闭。
