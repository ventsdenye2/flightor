...
The optional read-side Artifact envelope enrichment is passed through only for the matching accepted guideContentHash and stable activity ID. Original city identity clues remain `locationHint`; city center is never a venue coordinate. Stage two added the extension transport only; stage three now supplies independently persisted place identities via explicit authenticated GET/POST, without mutating the protected guide body. See [ADR 0026](adr/0026-place-identity-and-maps.md) for place records, cache and version guards. Media remains optional and independent; no image provider is connected.
