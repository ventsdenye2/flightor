...
Current implementation and live acceptance are tracked in [DEMO_STATUS.md](./DEMO_STATUS.md). On 2026-09-08 the final backend checks passed 514 ordinary tests and 26 PostgreSQL integration tests in separate runs; the backend TypeScript build and root tests passed. A real-mode WeChat build passed before the final login concurrency correction, which passed tests and type checking. Real conversational fare search, route generation, saving and restoration passed; the full guide remains partial and the final attempt timed out. Work stopped at the user's request. The new Planner delivery, atomic completion, artifact-version and client-session contracts are specified in [ADR 0011](./adr/0011-verified-delivery-and-workspace-consistency.md). Older gates below remain historical evidence, not proof of the current full flow.
