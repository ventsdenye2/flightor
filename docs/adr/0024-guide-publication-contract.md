...
- 2026-09-21 验收最小表达修正：无空格名称同样可做逐字前缀匹配；无 claim 的条目可保留其首个安全来源最多 800 字的搜索摘要引用，附 URL、记录时间和该引用的 SHA-256，明确标注“搜索摘要参考（非网页正文）”及适用性未审查。这是已绑定来源的实用参考，不使用模型 summary，不将搜索摘要升格为正文证据。已有 claim 无效/冲突时不走摘要兜底；引用 hash 错误不渲染。旧记录仍只读有限降级，测试重发副本不代表历史自动升级。
