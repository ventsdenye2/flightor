...
## 2026-09-22 第二项：正式概览与景点卡片

基于 main@8b51c99，正式适配器、概览、每日列表、活动详情及会话结果卡使用当前版本/locale 的接纳终稿；明确机票自备、状态分层、GET-only 恢复和显式准备/重试已接入。可选素材按 contentVersion/activityId 贯通，未接 Provider。字段映射、全部本轮检查、H5/微信 fixture 截图与实际边界集中在 [报告](PUBLICATION_UI_2026-09-22.md)。本轮模型/研究调用0、费用0；保留并排除其他任务改动，单独提交。不追认真实 API/真机或完整 G1 放行。


## 2026-09-22 地点身份与地图（第三阶段）

基线 main@8fd6691；修正双入口fixture身份/时段后，实现独立地点解析、缓存、公开API和正式地图。后端定向30项、生产适配35项、坐标/原生桥接/fixture测试及独立PostgreSQL通过；H5真实底图、双入口中英/日期/联动/刷新通过。微信原生标记和SDK联动通过，东京底图仍空白，保留未通过边界与紧凑收起入口。真实Nominatim15/24（5次超时、10次200），刷新不追加；模型/图片零调用。未重启Docker、未做G1全测。证据与下一阶段placeId接口见 [第三阶段记录](PLACES_MAP_2026-09-22.md)，不重写前两阶段历史。


## 2026-09-22 地图收尾阶段性提交

按用户要求基于1e097c4单独提交：文字与地点GET解耦、includePoints有界回退、分层脱敏诊断及验收脚本。定向后端13项、生产展示36项、地图回归与前端类型检查通过；H5真实底图/联动与故障注入通过，微信标记联动通过但底图根因未定。生产默认fetch新一次超时，POI累计16/24，模型/研究/图片0调用。最新离线小修晚于平台构建，不追认平台通过；当前不是第三阶段全平台完成。具体环境、截图、成功/失败证据、脚本用途及下一步见[情况说明](PLACES_MAP_FOLLOWUP_2026-09-22.md)。

## 2026-09-22 — 52cb1c3 地图限定收尾

隔离工作树保留用户改动；仅地点请求级代理与诊断脚本收尾。真实Provider请求成功与正式API地点409分开记录，固定东京窗口与SDK均空底图，未得到网络/鉴权根因，不改地图模板或重跑八项。费用新批次USD2，历史17次归档保留，新1次免费，用户随后取消地点次数上限；未调用模型。详见[结果与截图](MAP_CLOSEOUT_2026-09-22.md)。
# 2026-09-22 第四阶段图片

在`codex/place-media`独立工作树从`a980882`实施，保留主工作区及地图工作树未提交修改。真实Wikimedia上野公园/竹下通媒体已通过生产认证API+隔离PostgreSQL持久与正式H5封面/缩略图/详情解码；文字/行程为明确fixture。后端42项、前端36项+新增合并/浏览状态回归、独立DB和两端构建通过。19次服务端图片链路HTTP尝试（含失败与探测），已知费用0；模型/研究/地点查询0。微信照片页面/真机/正式public库未测，地图仍未解决且暂停排查。完整[报告](PLACE_MEDIA_2026-09-22.md)与[合同](../../adr/0027-place-media.md)为本阶段当前入口；不放行G1、不自动合main。

2026-09-25 10:17 DSH续验checkpoint：已提交并推送`babf6c7c55161f75be0f61145e58c71b0beab79e`。第四commit probe无有效正文、75.156秒失败，累计账本91模型/32搜索、US$6.20未知预留、余US$0.80，达到搜索上限后停止全部付费请求。用户追加US$2/32模型/8搜索此前已审计保留，不能重用该授权再次加额。修复官方单query schema/准入时序和DSH挑战页检测后backend29项、runtime6项通过；追加金额但不扩大模型上限的离线预算21项通过，build通过。最小probe改为独立attempt Trip和单景点范围，保留旧Goal/历史和原账本；尚未运行修复后的新真实probe。H5只读准备已恢复通过，截图/7份脱敏日志随提交保留；正式发送及accepted UI仍未通过。新增US$1/6搜索请求待用户确认，未应用。下述09:25前后记录为先前checkpoint，不覆盖本段当前状态。
