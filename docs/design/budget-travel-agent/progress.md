...
按 DPS 继续 G1：引用适用性边界已实现，接下来需对价格/开放时间取得可绑定的当前来源证据，并验证最终文字遵守引用限制，再补平台验收。不得仅增加附注就认定旧事实已正确，也不进入 M1。真实续验沿用原 US$2/48 模型/24 搜索账本；最近累计与剩余见本页上方。现有 runner 已支持跨进程续跑，不新建零消费账本。
