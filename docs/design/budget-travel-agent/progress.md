...
更新：2026-09-20。B0/B1 已提交 main `cabbf51`，B2 已提交 `d895d0e`，B3 已提交 `6649644`，B4 已提交 `1ce177b`，B5 已提交 `2c2cb1d`，G1 数据库验证已提交 `ee631a4`。当前阶段：G1 两条真实 Provider 保存/恢复契约通过，但第一例内容日期硬错误，G1 不放行。B2 仍默认关闭，平台验收未执行。保留用户原有未跟踪 `docs/demo/DEMO_MASTER.md`，不纳入提交。未修改运行模型/Provider 路由/依赖或现有环境文件。

## G1 探索范围与必需证据分离（2026-09-20）
...
按 DPS 继续 **G1 修复与复验**：先处理本次真实暴露的目标约束归属、活动日期证据门槛、canonical 地点复用及修复回合问题，再用保留样本做确定性回归与同案例实时复验，补平台展示。两条保存/恢复链路已用真实 Provider 验证，但自备机票内容硬失败不能放行。B4 首结果/API 时差和 B5 span 不等于 H5/微信体验或 A/B 提速成绩。

G1 质量及平台门槛通过后才启动正式时间测量与多案例批次。用户本批 US$2 已使用/预留 US$0.517287；剩余 US$1.482713 只属本批边界，不能重建账本把它当作新的 US$2。后续实时复验必须承接原账本占用与调用次数；当前单次脚本不支持跨进程续跑。该授权不涵盖后续正式 A/B 批次。
