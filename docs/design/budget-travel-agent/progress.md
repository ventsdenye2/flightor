...
更新：2026-09-20。B0/B1 已提交 main `cabbf51`，B2 已提交 `d895d0e`，B3 已提交 `6649644`，B4 已提交 `1ce177b`，B5 已提交 `2c2cb1d`，G1 数据库验证已提交 `ee631a4`。当前阶段：G1 修复后两条真实 Provider 追加复验已完成，保存/恢复契约2/2通过，但第一例内容价格时效有误，完整G1不放行。B2 仍默认关闭，平台验收未执行。保留用户原有未跟踪 `docs/demo/DEMO_MASTER.md`，不纳入提交。未修改运行模型/Provider 路由/依赖或现有环境文件。
