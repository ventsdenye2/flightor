# 当前修改方案：FlightOR 后端与 DSH 实施状态

日期：2026-09-24。状态：**既有 B0–B5 已实施，B2 仍默认关闭；G1 仍未通过，地图排查暂停。当前授权的后端工作按 DSH D0→D4 顺序推进，任何阶段完成状态必须由本轮实现和验证证据确认。**历史 G1 真实批次阶段耗时及质量见 [G1 报告](G1_LIVE_2026-09-20.md)，平台与正式 A/B 未验收。DSH 的固定版本/插件/分阶段目标见 [实施方案](DSH_IMPLEMENTATION_PLAN_2026-09-24.md)，逐项进度见 [progress](progress.md) 与本轮实施报告。
...
- `travel_guide` 参数中的 `requiredEvidenceTypes` 是可选字段且无默认值。省略时兼容旧合同：`researchTypes` 的全部类别都必需；显式提供时必须是 `researchTypes` 子集，仅该子集是必需证据，`researchTypes` 其余部分表示探索范围；显式空数组表示没有额外类别覆盖要求。共享 `requiredGuideEvidenceTypes` helper 同时服务 Goal、save、durable completion、PlanningContext 与 repair feedback，不从 Research Artifact brief 反推，也不改变已接受参数。可选 event 一旦选入攻略仍检查 ADR 0020 的 temporalEvidence 日期门槛。
...
- `requiredEvidenceTypes` 与其他 Goal 参数一起进入 fingerprint、幂等和同轮不可变约束；接受后减少必需类别必须开启新的用户轮次。旧 Goal/Run 不迁移，旧/新 JSON 共用存储；旧 strict reader 可能无法读取新增字段，回滚旧代码前必须处理这一兼容边界。
...
2026-09-24 用户授权已将本路线的实施顺序明确为 D0→D4，详见 [DSH 实施方案](DSH_IMPLEMENTATION_PLAN_2026-09-24.md)。旧 A/B、M1/M2/G2 和“条件 C1”顺序保留为历史决策背景，不再是 DSH 工作的前置。实施继续受方案中的隔离、API 兼容、业务边界、预算和 Key 围栏约束；没有新凭证/预算时完成可离线实施的工作，并一次报告真实调用的配置缺口。不得据实施顺序推断 G1 通过，也不得自动切换生产默认或合并 main。

历史决策记录（2026-09-20）：当时提出若 B 已达体验/质量目标则保留现有 runtime；若仅瓶颈在 Provider 等待或证据质量，则换 Harness 无针对性；只有残留问题涉及上下文、工具协议、恢复能力或维护成本时才做隔离 C 试验。2026-09-24 的明确授权更新了当前执行顺序，以上条件保留作历史判断，不再约束 D0→D4；生产默认切换仍须后续独立决定并具备回滚路径。
