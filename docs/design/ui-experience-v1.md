...
Artifact缓存按id+locale隔离，Plan/详情的请求key包含locale；语言切换后的迟到结果不可替换新语言视图。GET及刷新只读缓存/持久终稿；另一语言首次需求对已接纳终稿发起专用本地化POST，不研究/规划。接纳文本不再经过网页标题替换或每卡追加泛化免责声明，预算未知说明单独保留。图片、地图仍为空，后续以contentVersion+activityId独立扩展，不改正文hash。合同见[ADR 0025](../adr/0025-bounded-guide-finalization.md)。
