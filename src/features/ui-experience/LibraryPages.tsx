...
export function CloudTripsPage(props: CloudTripsPageProps) {
  ...
  return <>
    ...
    <View className='ux-scroll ui-page lb-cloud-page'>
      <Text className='ui-display'>{en ? 'View and continue your travel plans' : '查看和继续你的旅行计划'}</Text>
      <Text className='ux-muted'>{en ? 'Your saved plans, ready to pick up again.' : '查看已保存的行程，或继续上次的规划。'}</Text>
      {!props.signedIn ? <EmptyState icon='calendar' title={en ? 'Sign in to view your trips' : '登录后查看行程'} description={en ? 'Sign in to view your trips and continue previous conversations.' : '登录后查看已保存的行程，从上次的对话继续安排。'} actionLabel={en ? 'Sign in' : '登录与同步'} onAction={props.onLogin} /> : <>
        ...
        {!loading && !error && !trips.length ? <EmptyState icon='calendar' title={filter === 'archived' ? (en ? 'No archived trips yet' : '暂无归档行程') : (en ? 'No trips yet' : '暂无行程')} description={filter === 'archived' ? (en ? 'Archived trips stay here and can be restored anytime.' : '归档后的行程会留在这里，随时可以恢复。') : (en ? 'No trips in this category yet. Start with a place you want to visit.' : '还没有此类行程。选择出发地和日期，开始规划。')} actionLabel={filter === 'archived' ? (en ? 'View all trips' : '查看全部行程') : (en ? 'Start planning' : '开始规划')} onAction={filter === 'archived' ? () => props.onFilter(undefined) : props.onPlan} /> : null}
