...
export default function ProductionExplorePage({ items, category, onCategoryChange, loading, error, onRetry, hasMore, onLoadMore, busy, startError, onStart, onOpenSource }: ProductionExplorePageProps) {
    ...
    return <>
      ...
      <View className='ux-scroll ux-explore-detail' key={selected.id}>
        ...
        <View className='ux-explore-detail-body'>
          ...
          {template.experienceGoals.length > 0 && <><SectionHeading title='行程亮点'  /><View className='ux-explore-highlights'>{template.experienceGoals.map((goal, index) => <View className='ux-explore-highlight' key={`${index}-${goal}`}><Text className='ux-explore-index'>{String(index + 1).padStart(2, '0')}</Text><View><Text className='ux-explore-highlight-title'>{goal}</Text></View></View>)}</View></>}
      ...
      <View className='ux-explore-detail-action'><Button className='ux-primary' disabled={busy} onClick={() => onStart(selected)}>{busy ? '正在打开规划…' : '按此行程规划'}<Icon name='arrow-right' /></Button></View>
    </>
  }
  ...
  return <>
    ...
    <View className='ux-scroll ux-explore-page'>
      <View className='ux-explore-opening'><Text className='ux-title'>下一程，去哪里？</Text><Text className='ux-explore-subtitle'>看看目的地、活动和参考行程。</Text></View>
      ...
      <View className='ux-explore-results'>
        {remaining.length > 0 && <><SectionHeading title={query.trim() ? '找到这些旅行灵感' : '更多旅行灵感'} caption={query.trim() ? `${results.length} 份旅行灵感` : ''} /><View className='ux-explore-grid'>{remaining.map(item => <View className='ux-explore-card' key={item.id}><Button className='ux-explore-card-open' onClick={() => setSelectedId(item.id)}><Photo description={`${destinations(item) || '目的地'} · 图片暂未提供`} className='ux-explore-card-photo' retry={false} /><View className='ux-explore-card-copy'><Text className='ux-explore-card-category'>{`${categories[item.template.category] || item.template.category} · 约 ${item.template.suggestedDays} 天`}</Text><Text className='ux-explore-card-title'>{item.template.title}</Text><Text className='ux-muted'>{item.template.summary}</Text></View></Button></View>)}</View></>}
        {!loading && !error && results.length === 0 && <EmptyState title={query.trim() ? '未找到匹配的旅行灵感' : '暂时还没有旅行灵感'} description={query.trim() ? hasMore ? '可继续加载更多灵感，或试试其他目的地与关键词。' : '试试其他目的地或关键词，也可以清除搜索重新看看。' : '当前没有可用的旅行灵感，可以稍后再来看看。'} actionLabel={query.trim() ? '清除搜索' : '重新加载'} onAction={query.trim() ? () => setQuery('') : onRetry} />}
