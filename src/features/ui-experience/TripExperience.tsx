  ...
  return <View className={`ux-app ${embedded ? 'ux-embedded' : ''}`}>
    ...
    <View className='ux-scroll' key={tab}>
      ...
      {tab === 'overview' ? <View className='ux-overview'>
        ...
        {trip.supportingEvidence?.length ? <View className='ux-guide-evidence'><Text className='ux-section-title'>实用与补充信息</Text>{trip.supportingEvidence.map((item, index) => <View key={`${item.title}-${index}`} className='ux-guide-evidence-item'><Text className='ux-section-title'>{item.title}</Text><Text className='ux-muted'>{item.description}</Text><Text className='ux-caption'>{item.category} · {item.verification === 'verified' ? '已核验' : item.verification === 'partial' ? '部分核验' : item.verification === 'stale' ? '资料已过期' : '待核验'}{item.destinations.length ? ` · ${item.destinations.join('、')}` : ''}</Text></View>)}</View> : null}
      </View> : tab === 'days' ? <>
