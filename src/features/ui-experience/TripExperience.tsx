...
function TripContent({ trip, imageError = false, onNavigate, onOpenSource, embedded = false, onBack, saved: controlledSaved, onSavedChange, onProductionEdit, onSave, production = false, onContinuePlanning, initialTab = 'overview' }: TripExperienceProps) {
  ...
  return <View className={`ux-app ${embedded ? 'ux-embedded' : ''}`}>
    ...
    <View className='ux-scroll' key={tab}>
      ...
      {partial ? <View className='ux-progress' role='status'><View className='ux-progress-dot' /><View><Text>{production ? '已有保存内容，交付状态仍待确认' : trip.flights.length ? '已有航班参考，日程仍在补充' : '日程正在补充'}</Text><Text className='ux-caption'>{completed.length ? `已有 ${completed.join('、')} 安排，可以先查看和调整。` : '每日安排尚未完成，请稍后再查看。'}</Text></View></View> : null}
    ...
    {sheet ? <View className='ux-modal' onClick={closeSheet}>
      <View className='ux-sheet' role='dialog' aria-modal='true' ariaLabel={sheet.kind === 'activity' ? sheet.activity.name : sheet.kind === 'adjust' ? '调整这一天' : '图片与来源'} onClick={event => event.stopPropagation()}>
        ...
        {sheet.kind === 'activity' ? <>
          ...
          <Button className='ux-adjust-option' disabled={!onProductionEdit && !candidate} onClick={() => { if (onProductionEdit) updateDay(day.activities, '在这一天增加一个合适的停留'); else if (candidate) updateDay([...day.activities, candidate], `已加入${candidate.name}，可以撤销`) }}><Text className='ux-section-title'>加一点风景</Text><Text className='ux-muted'>{onProductionEdit ? '请让 Planner 为今天安排一个合适的停留。' : candidate ? `把${candidate.name}加入今天，具体时间可以再确认。` : '当前没有其他可用候选，已有安排仍可调整。'}</Text><Icon name='arrow-right' /></Button>
          <Text className='ux-caption'>{onProductionEdit ? '调整会发送给 Planner，并在新的已保存快照返回后更新。' : '当前使用此行程的固定候选演示调整，尚未连接行程生成服务。'}</Text>
        </> : <>
