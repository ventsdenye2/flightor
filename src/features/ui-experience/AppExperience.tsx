...
import { ExplorePage, exploreItems } from './ExplorePage'
...
export default function AppExperience({ scenario = 'ready', requestedPage = 'plan', navigationVersion = 0, onOpenSource }: { scenario?: Scenario; requestedPage?: PreviewPage; navigationVersion?: number; onOpenSource?: (url: string) => void }) {
  ...
  const savedItems = exploreItems.filter(item => savedIds.includes(item.id)).map(item => ({ id: item.id, title: item.title, subtitle: item.subtitle, photo: media[item.photo] }))
  return <View className={`ux-app ${largeText ? 'is-comfortable' : ''}`}>
    <View className='ux-screen' style={{ display: visible('plan') ? 'flex' : 'none' }}><PlannerPage key={prompt.revision} initialPrompt={prompt.text} onOpenTrip={openTrip} onSearchFlights={() => navigate('flights')} /></View>
    <View className='ux-screen' style={{ display: visible('explore') ? 'flex' : 'none' }}><ExplorePage onPlan={planWith} savedIds={savedIds} onToggleSave={toggleItem} onOpenSource={onOpenSource} /></View>
    <View className='ux-screen' style={{ display: visible('trips') ? 'flex' : 'none' }}><TripsPage onOpenTrip={openTrip} onPlan={() => planWith('')} archived={archived} onArchive={setArchived} /></View>
    ...
    {tripOpened ? <View className='ux-screen' style={{ display: visible('trip') ? 'flex' : 'none' }}><TripExperience key={scenario} scenario={scenario} embedded onBack={closeOverlay} saved={tripSaved} onSavedChange={setTripSaved} onOpenSource={onOpenSource} /></View> : null}
    {overlayStack.includes('flights') ? <View className='ux-screen' style={{ display: visible('flights') ? 'flex' : 'none' }}><FlightExplorer onBack={closeOverlay} onOpenTrip={openTrip} onCreateAlert={alert => {
    ...
    {overlayStack.includes('inspiration') ? <View className='ux-screen' style={{ display: visible('inspiration') ? 'flex' : 'none' }}><ExplorePage key={inspirationId} initialItemId={inspirationId} onBack={closeOverlay} onPlan={planWith} savedIds={savedIds} onToggleSave={toggleItem} onOpenSource={onOpenSource} /></View> : null}
