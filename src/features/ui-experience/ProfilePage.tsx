...
export function CloudProfilePage({ profile, locale, preferences, onLogin, onLogout, onTrips, onAlerts, onPlan, onLocaleChange, onAbout }: CloudProfilePageProps) {
  ...
  return <>
    ...
    <View className='ux-scroll'>
      {page === 'home' ? <>
        <View className='pr-identity'><View className='pr-avatar'>{profile?.avatarUrl ? <Image className='pr-cloud-avatar' src={profile.avatarUrl} mode='aspectFill' /> : <Icon name='user' />}</View><View><Text className='ui-display'>{profile ? profile.nickname || (en ? 'Traveler' : '旅行者') : (en ? 'Hello, traveler' : '你好，旅行者')}</Text><Text className='ux-muted'>{profile ? accountLabel : (en ? 'Sign in to save trips and travel preferences.' : '登录后保存行程和旅行偏好。')}</Text>{!profile ? <Button className='ux-text-button' onClick={onLogin}>{en ? 'Sign in & sync' : '登录与同步'}<Icon name='chevron-right' /></Button> : null}</View></View>
        ...
        <View className='ui-page pr-home-body'><SectionHeading title={en ? 'Travel preferences' : '旅行偏好'} /><View className='pr-preference-card'><View><Text className='ux-section-title'>{en ? 'Set your travel preferences' : '设置常用偏好'}</Text><Text className='ux-muted'>{en ? 'Your usual starting point, interests and travel pace.' : '常用出发地、感兴趣的去处，以及舒服的旅行节奏。'}</Text><Button className='ux-text-button' onClick={profile ? () => setPage('preferences') : onLogin}>{profile ? (en ? 'Edit travel preferences' : '编辑旅行偏好') : (en ? 'Sign in to save preferences' : '登录后保存偏好')}<Icon name='arrow-right' /></Button></View><Icon name='compass' /></View>
          <MenuRow icon='settings' title={en ? 'Settings' : '设置'} description={en ? 'Language and account' : '界面语言、登录账户'} onClick={() => setPage('settings')} />
          <MenuRow icon='info' title={en ? 'About FlightOR' : '关于 FlightOR'} description={en ? 'Product information and guidance' : '产品介绍与使用说明'} onClick={onAbout} />
        </View>
