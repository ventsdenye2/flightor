...
export function Photo({ src, description, className = '', forceError = false, retry = true }: {
  src: string; description: string; className?: string; forceError?: boolean; retry?: boolean
}) {
  const [failed, setFailed] = useState(forceError)
  const [attempt, setAttempt] = useState(0)
  useEffect(() => { setFailed(forceError) }, [src, forceError])
  if (failed) return <View className={`ux-photo-fallback ${className}`}>
    <Icon name='image' /><Text>{description}</Text>
    {retry ? <Button className='ux-text-button' onClick={() => { setFailed(false); setAttempt(n => n + 1) }}>重新加载图片</Button> : null}
  </View>
