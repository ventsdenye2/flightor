...
export default function TripMap({points,selectedId,onSelect,locale,orderLine,flightLines=[],mapKey}:TripMapProps){
 ...
 useEffect(()=>{
  ...
  recordMapDiagnostic('map-config','read',null,'pending')
  const configRequest=request<{tileUrl:string}>({url:'/v1/map-config',retry:0}).then(config=>{
   recordMapDiagnostic('map-config','read',null,'succeeded');return config
  },error=>{recordMapDiagnostic('map-config','read',error,'failed');throw error})
  void Promise.all([import('leaflet'),configRequest]).then(([L,config])=>{
   ...
   L.tileLayer(config.tileUrl,{maxZoom:19,attribution:'&copy; <a href="https://www.openstreetmap.org/copyright" target="_blank">OpenStreetMap contributors</a>'})
    .on('tileload',()=>{loaded++;if(active){setReady(true);if(loaded===1)recordMapDiagnostic('h5-tiles','tileload',{basemap:'tile_loaded'},'succeeded')}})
    .on('tileerror',event=>recordMapDiagnostic('h5-tiles','tileerror',{error:event.error,coords:event.coords},'failed')).addTo(instance)
   for(const p of points){
    ...
    marker.bindTooltip(label,{direction:'top',permanent:selectedId===p.id}).on('click',()=>{recordMapDiagnostic('h5','marker_interaction',{activityId:p.id});select.current?.(p.id)}).addTo(instance)
   }
   ...
   try {instance.fitBounds(L.latLngBounds(points.map(p=>[p.latitude,p.longitude])),{padding:[28,28],maxZoom:15});recordMapDiagnostic('h5','fitBounds',null,'succeeded')}
   catch(error){recordMapDiagnostic('h5','fitBounds',error,'failed');instance.setView([points[0].latitude,points[0].longitude],13)}
   timer=setTimeout(()=>{if(active&&!loaded){recordMapDiagnostic('h5-tiles','timeout',null,'failed');setFailed(true);instance.remove();map.current=undefined}},8000)
  }).catch(error=>{recordMapDiagnostic('h5','creation',error,'failed');if(active)setFailed(true)})
