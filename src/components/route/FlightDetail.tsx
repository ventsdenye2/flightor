...
import { displayOffer, record, records } from '../artifacts/payload'
...
export function FlightDetail({ artifact, offerId }: { artifact: ArtifactEnvelope; offerId: string }) {
  ...
  const raw = payload && [...records(payload.offers, 100), ...records(payload.results, 31).flatMap(result => records(result.offers, 100))].find(o => o.id === offerId)
  const offer = displayOffer(raw)
